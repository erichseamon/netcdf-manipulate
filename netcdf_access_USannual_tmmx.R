#------------------------------------------------------------------------#
# TITLE:        netcdf_access_USannual_tmmx.R
#
# AUTHOR:       Erich Seamon
#
...
for (h in 2001:2015) {
  ...
  climatevar_short <- "tmmx"
  climatevar <- "daily_maximum_temperature"
  
  nc <- nc_open(paste("/dmine/data/netcdf/tmmx_", h, "_average.nc", sep=""))
  ...
  #for (i in 1:nt) {
    ...
    dt<-ncvar_get(nc, varid = 'air_temperature', start = start, count = count)
  ...
  colnames(newmatrix) <- c("FIPS", "tmmx")
  county_tmmx <- merge(counties, newmatrix, by=c("FIPS"))
  
  county_tmmx$tmmx <- as.numeric(as.character((county_tmmx$tmmx)))
  
  
  
  writeOGR(obj=county_tmmx, dsn="/dmine/data/GRIDMET_USannual", layer=paste("county_tmmx_", h, sep=""), driver="ESRI Shapefile") # this is in geographical projection
  
  
  
  
  pal <- colorNumeric(brewer.pal(11, "BrBG"),  na.color = "#ffffff", domain = eval(parse(text=paste("county_pr$", "tmmx", sep=""))))
  
  exte <- as.vector(extent(counties))
  
  label <- paste(sep = "<br/>", county_pr$NAME, round(eval(parse(text=paste("county_tmmx$", "tmmx", sep=""))), 0))
  markers <- data.frame(label)
  labs <- as.list(eval(parse(text=paste("county_tmmx$", "tmmx", sep=""))))

#map <- leaflet(data = county_tmmx) %>% addProviderTiles("Stamen.TonerLite")  %>% fitBounds(exte[1], exte[3], exte[2], exte[4]) %>% addPolygons(color = ~pal(eval(parse(text=paste("county_tmmx$", "tmmx", sep="")))),  popup = markers$label, weight = 1)  
  #addLegend(pal = pal, values = ~na.omit(eval(parse(text=paste("county_tmmx$", "tmmx", sep="")))),  labels = c("1", "2"), opacity = .5,
