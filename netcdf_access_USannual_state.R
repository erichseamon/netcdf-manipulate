climate_annual_state <- function(year, climvar){
  ...
  plot(eval(parse(text=paste("data$", climvar, sep=""))) ~ data$STATE_NAME, xlab = "", main = paste(climvar, " for ", year, " by county, per state", sep=""), ylab = paste(climvar, " for ", year, sep=""),  las=3)
  stripchart(eval(parse(text=paste(climvar, sep=""))) ~ STATE_NAME, data = data, vertical = TRUE, 
             method = "jitter", add = TRUE, pch = 1, cex = .3, col = 'blue')
