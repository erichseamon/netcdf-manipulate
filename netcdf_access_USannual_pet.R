...
for (h in 2001:2015) {
  ...
  climatevar_short <- "pet"
  climatevar <- "potential_evapotranspiration"
  
  nc <- nc_open(paste("/dmine/data/netcdf/pet_", h, "_average.nc", sep=""))
  ...
  #for (i in 1:nt) {
    ...
    dt<-ncvar_get(nc, varid = 'potential_evapotranspiration', start = start, count = count)
  ...
  colnames(newmatrix) <- c("FIPS", "pet")
  county_pet <- merge(counties, newmatrix, by=c("FIPS"))
  
  county_pet$pet <- as.numeric(as.character((county_pet$pet)))
  
  
  
  writeOGR(obj=county_pet, dsn="/dmine/data/GRIDMET_USannual", layer=paste("county_pet_", h, sep=""), driver="ESRI Shapefile") # this is in geographical projection
  
  
  
  
  pal <- colorNumeric(brewer.pal(11, "BrBG"),  na.color = "#ffffff", domain = eval(parse(text=paste("county_pr$", "pet", sep=""))))
  
  exte <- as.vector(extent(counties))
  
  label <- paste(sep = "<br/>", county_pr$NAME, round(eval(parse(text=paste("county_pet$", "pet", sep=""))), 0))
  markers <- data.frame(label)
  labs <- as.list(eval(parse(text=paste("county_pet$", "pet", sep=""))))

#map <- leaflet(data = county_pet) %>% addProviderTiles("Stamen.TonerLite")  %>% fitBounds(exte[1], exte[3], exte[2], exte[4]) %>% addPolygons(color = ~pal(eval(parse(text=paste("county_pet$", "pet", sep="")))),  popup = markers$label, weight = 1)  
  #addLegend(pal = pal, values = ~na.omit(eval(parse(text=paste("county_pet$", "pet", sep="")))),  labels = c("1", "2"), opacity = .5,
